    ...
    <?php
        ...
        if($usuario = $statement->fetch()){
            ?>
                ...
                <a href="login.php"><button>Deslogar</button></a>
                <br><br>
                <form method="post">
                    <button type="submit" name="botaofilan">Exibir fila normal</button>
                    <button type="submit" name="botaofilap">Exibir fila preferencial</button>
                    <button type="submit" name="botaochamarfilan">Chamar próximo da fila normal</button>
                    <button type="submit" name="botaochamarfilap">Chamar próximo da fila preferencial</button>
            ...
            function chamarProximo($fila) {
                ...
                if ($dados = $statement->fetch()) {
                    ...
                    ?> <h3>Próximo chamado com sucesso!</h3> <?php
                    ?> <h3>Nome: <?php echo $nome ?></h3> <?php
                    ?> <h3>Senha: <?php echo $senha." ".$tiposenha ?></h3> <?php
                } else {
                    ?> <h3>A fila chamada está vazia!</h3> <?php
                }
            }
